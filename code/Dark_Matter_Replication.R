...
d1 <- cs %>% filter(!is.na(cum_dm_bn), !is.na(cum_oca_bn))
...
d2 <- cs %>% filter(!is.na(dm_exp_gdp), !is.na(cum_oca_gdp))
...
d4 <- panel %>%
  filter(year >= 1980, year <= 2004, !is.na(ca_dm_gdp), !is.na(output_vol))
...
tex4 <- tryCatch(
  capture.output(
    stargazer(t4[[1]], t4[[2]], t4[[3]], t4[[4]], t4[[5]], t4[[6]],
              column.labels    = names(t4),
              title            = "Sources of Dark Matter: Panel Evidence (1980--2004)",
              label            = "tab:table4",
              dep.var.labels   = "Dark matter CA (\\% GDP)",
              covariate.labels = c("FDI liabilities (\\% GDP)",
                                   "FDI assets (\\% GDP)",
                                   "Output volatility", "Constant"),
              omit.stat        = c("f", "ser", "adj.rsq"),
              notes            = paste0("Standard errors in parentheses. ",
                                        "* p$<$0.10, ** p$<$0.05, *** p$<$0.01. ",
                                        "Restr. = excl. OPEC and HIPC countries. ",
                                        "Ind. = 21 industrial countries. ",
                                        "FE = country fixed effects."),
              notes.align      = "l",
              style            = "aer",
              type             = "latex")),
  error = function(e) { message("Table 4 failed: ", e$message); NULL }
)

if (!is.null(tex4))
  compile_table(paste(tex4, collapse = "\n"), "table4", landscape = TRUE)
